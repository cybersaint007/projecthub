...
Project with code 'PH-CORE' already exists. Import stopped.
Import failed: Duplicate project code: PH-CORE
```
...
3. **JSON 結構錯誤**
   ```
   Invalid JSON structure: missing "project" or "epics"
   ```
...
5. **專案代碼重複**
   ```
   Project with code '{code}' already exists. Import stopped.
   ```
...
- `projects.code` (string, nullable, unique, indexed)
...
2. **確保有測試使用者**
   ...
   例如，如果 JSON 中使用 `james.lee@fincosoft.com`，需要先建立該使用者。
...
- 不會建立任何新記錄（transaction rollback）
