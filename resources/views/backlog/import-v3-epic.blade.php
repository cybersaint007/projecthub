<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('ui.import_epic') }} &mdash; {{ $project->name }}</h2>
                <p class="text-sm text-gray-500 mt-1">Add or update an epic (and its tasks) inside this project. Safe to re-run — existing records matched by <code class="bg-gray-100 px-1 rounded">id</code> or <code class="bg-gray-100 px-1 rounded">external_key</code> are updated, not duplicated.</p>
            </div>
    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="bg-white shadow-sm sm:rounded-lg p-6">
            ...
            <p class="text-sm text-gray-500 mb-4">
                Must include <code class="bg-gray-100 px-1 rounded">schema_version: "3.0"</code> and
                <code class="bg-gray-100 px-1 rounded">epic</code> with a <code class="bg-gray-100 px-1 rounded">title</code>.
                Use the JSON produced by <strong>{{ __('ui.export_epic') }}</strong> on any epic page.
            </p>

            <form method="POST" action="{{ route('backlog.epic.import-v3.apply', $project) }}" enctype="multipart/form-data">
                @csrf
