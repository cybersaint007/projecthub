<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('ui.edit_task') }}: {{ $task->title }}</h2>
    ...
    <div class="max-w-3xl mx-auto bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
        <form method="POST" action="{{ route('tasks.update', $task) }}">
            ...
            <div class="mb-4">
                <x-input-label for="context" :value="__('ui.context')" />
            ...
            <div class="mb-4">
                <x-input-label for="instructions" :value="__('ui.instructions')" />
            ...
            <div class="flex items-center gap-4">
                <x-primary-button>{{ __('ui.update_task') }}</x-primary-button>
                <a href="{{ route('tasks.show', $task) }}" class="text-sm text-gray-600 hover:underline">{{ __('ui.cancel') }}</a>
            </div>
        </form>
    </div>
</x-app-layout>
