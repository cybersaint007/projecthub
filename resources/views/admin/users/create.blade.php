<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('ui.create_user') }}</h2>
    ...
    <div class="max-w-md mx-auto bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
        <p class="text-sm text-gray-500 mb-4">{{ __('ui.auto_password_note') }}</p>
        ...
        <form method="POST" action="{{ route('admin.users.store') }}">
            ...
            <x-primary-button>{{ __('ui.create_user') }}</x-primary-button>
        </form>
    </div>
</x-app-layout>
